...
// 🔥 IMPORTANDO O CÉREBRO DE PERMISSÕES
...
function card(href, icon, title, sub, disabled=false) {
...
export async function renderDashboard(root) {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (userData.status !== 'approved' && userData.status !== 'active' && userData.global_role !== 'ADMIN_GERAL') {
             // Aceitamos approved, active ou se o cara for admin geral
             // (mantendo compatibilidade com cadastros antigos)
        ...
        // Desenha a tela usando APENAS o nome. Sem o botão "Sair" repetido.
  ...
  function buildDashboardUI(userName) {
    ...
    // 2. CADASTRO
    // Verifica se ele pode ver pelo menos UMA das páginas de cadastro
    ...
    // Verifica se ele pode ver pelo menos UMA das ferramentas
    if (canViewPage('simulador') || canViewPage('csv') || canViewPage('resultados') || canViewPage('status')) {
      html += `
        <div class="row">
    ...
    // 4. ADMINISTRAÇÃO 
    // Só o ADMIN_GERAL tem a view "gestao" liberada no permissions.js
    if (canViewPage('gestao')) {
      html += `
        <div class="row">
          <h2>Administração do Sistema</h2>
          <div class="cards">
            ${card('#/admin/users', icons.admin, 'Gestão de Acessos', 'Aprovar Oficiais e definir funções')}
          </div>
        </div>
      `;
    }
