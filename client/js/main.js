...
// Importa o cérebro das permissões
...
const routes = {
  ...
  '#/teams':        () => import('./pages/teams.js').then(m => m.renderTeams), // 🔥 ROTA DE PARES E EQUIPES
  ...
  // 🔥 MÓDULO DE ACESSO OPERACIONAL (MESAS E CÂMARA)
...
export async function navigate(hash) {
  ...
  // 🛡️ A PORTA DO COFRE: Verifica a permissão ANTES de carregar a página
  ...
  try {
    ...
    if (routeLoader) {
      ...
      // 🔥 LISTA DE ROTAS QUE EXIGEM O ID DA COMPETIÇÃO
      const compRoutes = [
         '#/competitions/class', '#/competitions/athletes', '#/competitions/officials', 
         '#/competitions/schedule', '#/competitions/match-sheets', '#/competitions/violations', 
...
// 🔥 SÓ INICIA O ROTEAMENTO DEPOIS QUE O FIREBASE DISSER QUAL É A PERMISSÃO DO USUÁRIO (Prevenindo o Erro de F5)
...
/* ========= MENU HAMBÚRGUER (ORIGINAL DO FICHEIRO) ========= */
(function () {
